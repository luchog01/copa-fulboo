...
import { updateMatch, deleteMatch } from "@/repositories/matches";
...
export async function updateMatchAction(
  ...
  const playerEntries = [
    ...homePlayers.map((id) => ({ playerId: id, team: "home" as const, isMvp: mvpPlayerId === id, matchId })),
    ...awayPlayers.map((id) => ({ playerId: id, team: "away" as const, isMvp: mvpPlayerId === id, matchId })),
  ];
...
export async function deleteMatchAction(matchId: number): Promise<never> {
  await requireAuth();
